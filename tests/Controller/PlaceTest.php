...
class PlaceTest extends ControllerBaseCase {
    // Change this to HTTP_OK when the site is public.
    private const ANON_RESPONSE_CODE = Response::HTTP_FOUND;

    private const TYPEAHEAD_QUERY = 'place';
    ...
    public function testAnonSearch() : void {
        $repo = $this->createMock(PlaceRepository::class);
        $repo->method('searchQuery')->willReturn([$this->getReference('place.1')]);
        $this->client->disableReboot();
        $this->client->getContainer()->set('test.' . PlaceRepository::class, $repo);

        $crawler = $this->client->request('GET', '/place/search');
        $this->assertSame(self::ANON_RESPONSE_CODE, $this->client->getResponse()->getStatusCode());
        if (self::ANON_RESPONSE_CODE === Response::HTTP_FOUND) {
            // If authentication is required stop here.
            return;
    ...
    public function testAdminEdit() : void {
        ...
        $form = $formCrawler->selectButton('Save')->form([
            ...
            'place[sortableName]' => 'Updated SortableName',
            'place[geoNamesId]' => 'Updated GeonamesId',
            'place[regionName]' => 'Updated RegionName',
            'place[countryName]' => 'Updated CountryName',
        ...
        $this->assertSame(1, $responseCrawler->filter('td:contains("Updated GeonamesId")')->count());
        $this->assertSame(1, $responseCrawler->filter('td:contains("Updated RegionName")')->count());
        $this->assertSame(1, $responseCrawler->filter('td:contains("Updated CountryName")')->count());
    ...
    public function testAdminNew() : void {
        ...
        $form = $formCrawler->selectButton('Save')->form([
            'place[name]' => 'New Name',
            'place[sortableName]' => 'New SortableName',
            'place[geoNamesId]' => 'New GeonamesId',
            'place[regionName]' => 'New RegionName',
            'place[countryName]' => 'New CountryName',
            'place[latitude]' => 'New Latitude',
            'place[longitude]' => 'New Longitude',
            'place[description]' => 'New Description',
            'place[notes]' => 'New Notes',
        ]);
        ...
        $this->assertSame(1, $responseCrawler->filter('td:contains("New GeonamesId")')->count());
        $this->assertSame(1, $responseCrawler->filter('td:contains("New RegionName")')->count());
        $this->assertSame(1, $responseCrawler->filter('td:contains("New CountryName")')->count());
