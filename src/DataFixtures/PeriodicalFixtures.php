...
class PeriodicalFixtures extends Fixture implements DependentFixtureInterface {
    ...
    public function load(ObjectManager $em) : void {
        for ($i = 1; $i <= 4; $i++) {
            ...
            $fixture->setRunDates('RunDates ' . $i);
            $fixture->setContinuedFrom("<p>This is paragraph {$i}</p>");
            $fixture->setContinuedBy("<p>This is paragraph {$i}</p>");
            $fixture->setDateyear($this->getReference('dateyear.' . $i));
            $fixture->setLocation($this->getReference('location.' . $i));
            $em->persist($fixture);
            $this->setReference('periodical.' . $i, $fixture);
        }
        $em->flush();
    }
