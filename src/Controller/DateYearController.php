...
class DateYearController extends AbstractController implements PaginatorAwareInterface
{
     ...
     * @param DateYearRepository $dateYearRepository
     *
     * @Template()
     *
     * @return array
    ...
    public function search(Request $request, DateYearRepository $dateYearRepository) {
        ...
        if ($q) {
            ...
            $dateYears = $this->paginator->paginate($query, $request->query->getInt('page', 1), $this->getParameter('page_size'), array('wrap-queries'=>true));
        } else {
            $dateYears = [];
        }
